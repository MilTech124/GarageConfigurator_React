...
function CalcMain({ selectedOptions, price, setPrice }) {
  ...
  const roofKey = roof === "spad ty�" || roof === "spad tył" ? "spad tyl" : roof;
  ...
  const calcCarportPrice = () => {
    ...
    const variablePrice = 1000;
    const typePrice =
      carportType === "brak"
        ? 1000
        : carportType === "oblachowane"
        ? 2000
        : carportType === "azury" || carportType === "mix"
        ? 2500
        : null;

    if (carport) {
      resault =
        ((carportWidth - StandardWith) / StandardPaidWith) * pricePerMeter +
        typePrice +
        variablePrice;
    }
    return resault;
  ...
  const calcGutterPrice = () => {
    ...
    if (roof === "dwuspad" || roof === "dwuspad przod-tył" || roof === "dwuspad przod-ty�") {
      ...
      if (
        carport &&
        (roof === "dwuspad przod-tył" || roof === "dwuspad przod-ty�") &&
  ...
  return (
    <div>
      <p className="hidden text-4xl max-sm:text-base md:pt-5 text-red-800 font-bold">
        Cena:
        <span className="underline ml-5 font-black">{price} zł</span>
      </p>
      <p className="hidden md:text-sm text-xs md:pb-2">
